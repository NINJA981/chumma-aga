...
import { createLeadSchema, leadFiltersSchema, updateLeadSchema } from '../validators/lead.validator.js';
...
router.get('/', validateRequest(leadFiltersSchema), asyncHandler(async (req: Request, res: Response) => {
    const { page = 1, limit = 50, status, assignedTo, search } = req.query;
    const offset = (Number(page) - 1) * Number(limit);

    let whereClause = 'WHERE l.org_id = $1 AND l.deleted_at IS NULL';
...
router.post('/', validateRequest(createLeadSchema), asyncHandler(async (req: Request, res: Response) => {
    const data = req.body;
...
router.delete('/:id', asyncHandler(async (req: Request, res: Response) => {
    const lead = queryOne(
        pgToSqlite('SELECT id FROM leads WHERE id = $1 AND org_id = $2 AND deleted_at IS NULL'),
        [req.params.id, req.user!.orgId]
    );
    ...
    execute(
        pgToSqlite('UPDATE leads SET deleted_at = CURRENT_TIMESTAMP WHERE id = $1'),
        [req.params.id]
    );
...
router.get('/:id/optimal-time', asyncHandler(async (req: Request, res: Response) => {
    const lead = queryOne(
...
router.post('/import', requireManager, upload.single('file'), asyncHandler(async (req: Request, res: Response) => {
    if (!req.file) {
